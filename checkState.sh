...
if [ `ps -ef | grep collector | grep -v grep | wc -l` -ne 0 ]; then #  running
    collectorState="Running"
else
    collectorState="Not Running"
fi

if [ `ps -ef | grep agentServer | grep -v grep | wc -l` -ne 0 ]; then #  running
    agentServerState="Running"
else
    agentServerState="Not Running"
fi

if [ `ps -ef | grep adaptive | grep -v grep | wc -l` -ne 0 ]; then #  running
    adaptiveState="Running"
else
    adaptiveState="Not Running"
fi

if [ `ps -ef | grep vpnlogin_autoconnec | grep -v grep | wc -l` -ne 0 ]; then #  running
